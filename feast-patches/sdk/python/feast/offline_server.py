...
class OfflineServer(fl.FlightServerBase):
    ...
    def get_historical_features(self, command: dict, key: Optional[str] = None):
        ...
        try:
            if hasattr(self.store.config, "mlflow") and self.store.config.mlflow and self.store.config.mlflow.enabled:
                self.store.registry.record_training_run(
                    project=project,
                    run_id=command.get("mlflow_run_id", "server-retrieval"),
                    experiment_name=command.get("mlflow_experiment", ""),
                    tracking_uri=self.store.config.mlflow.tracking_uri,
                    feature_service_name=command.get("feature_service_name", ""),
                    feature_refs=feature_refs,
                )
        except Exception:
            pass
