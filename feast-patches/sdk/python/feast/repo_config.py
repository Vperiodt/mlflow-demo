...
class MlflowConfig(FeastBaseModel):
    """Configuration for MLflow experiment tracking integration.

    This enables automatic logging of feature metadata to MLflow when
    Feast operations like get_historical_features are called during training.
    ...
    tracking_uri: StrictStr = "http://localhost:5000"
    """ str: MLflow tracking server URI. """

    auto_log: StrictBool = True
    """ bool: Auto-log feature metadata (tags, params, contract) to MLflow runs. """

    lineage: StrictBool = True
    """ bool: Record training runs in registry for lineage tracking. """

    def to_mlflow_config(self):
        """Convert to feast.mlflow_integration.config.MlflowIntegrationConfig."""
        from feast.mlflow_integration.config import MlflowIntegrationConfig

        return MlflowIntegrationConfig(
            enabled=self.enabled,
...
class RepoConfig(FeastBaseModel):
    ...
    mlflow_config: Optional[MlflowConfig] = Field(None, alias="mlflow")
