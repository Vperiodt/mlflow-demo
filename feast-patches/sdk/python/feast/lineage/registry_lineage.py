...
class RegistryLineageGenerator:
    ...
    def _parse_direct_relationships(self, registry: Registry) -> List[EntityRelation]:
        ...
        # FeatureService -> MLflow Training Run relationships
        if hasattr(registry, "training_runs"):
            for training_run in registry.training_runs:
                run_label = f"{training_run.experiment_name}:{training_run.run_id[:8]}"
                relationships.append(
                    EntityRelation(
                        source=EntityReference(
                            FeastObjectType.FEATURE_SERVICE,
                            training_run.feature_service_name,
                        ),
                        target=EntityReference(
                            FeastObjectType.MLFLOW_RUN,
                            run_label,
                        ),
                    )
                )
