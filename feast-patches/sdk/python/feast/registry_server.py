...
class RegistryServer(RegistryServer_pb2_grpc.RegistryServerServicer):
    ...
    def RecordTrainingRun(self, request, context):
        self.proxied_registry.record_training_run(
            project=request.project,
            run_id=request.training_run.run_id,
            experiment_name=request.training_run.experiment_name,
            tracking_uri=request.training_run.tracking_uri,
            feature_service_name=request.training_run.feature_service_name,
            feature_refs=list(request.training_run.feature_refs),
        )
        return Empty()

    def ListTrainingRuns(self, request, context):
        training_runs = self.proxied_registry.list_training_runs(
            project=request.project,
            feature_service_name=request.feature_service_name if request.feature_service_name else None,
            allow_cache=request.allow_cache,
        )
        return RegistryServer_pb2.ListTrainingRunsResponse(
