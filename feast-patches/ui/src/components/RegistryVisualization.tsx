...
const getLightNodeColor = (type: FEAST_FCO_TYPES) => {
  switch (type) {
    ...
    case FEAST_FCO_TYPES.mlflowRun:
      return "#e3f2fd"; // Light blue (MLflow)
    default:
  ...
  const handleClick = () => {
    ...
    switch (data.type) {
      ...
      case FEAST_FCO_TYPES.featureService:
        ...
        break;
      case FEAST_FCO_TYPES.mlflowRun:
        if (data.metadata?.trackingUri && data.metadata?.runId) {
          window.open(
            `${data.metadata.trackingUri}/#/experiments/0/runs/${data.metadata.runId}`,
            "_blank",
          );
        }
        return;
...
const registryToFlow = (
  ...
  // MLflow training run nodes
  const trainingRuns = (objects as any).trainingRuns || [];
  trainingRuns.forEach((run: any) => {
    const runLabel = `${run.experimentName || ""}:${(run.runId || "").substring(0, 8)}`;
    nodes.push({
      id: `mlrun-${runLabel}`,
      type: "custom",
      data: {
        ...
        metadata: {
          ...
          trackingUri: run.trackingUri,
          featureServiceName: run.featureServiceName,
          featureRefs: run.featureRefs,
        },
        permissions: [],
      },
      position: { x: 0, y: 0 },
